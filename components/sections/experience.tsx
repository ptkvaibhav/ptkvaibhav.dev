  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-50 dark:bg-emerald-950/60 px-3.5 py-1.5 text-xs font-bold text-emerald-800 dark:text-emerald-300 shadow-sm">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected
                    ? "border-cyan-500 bg-cyan-500/10 dark:border-cyan-400/90 dark:bg-cyan-950/40 shadow-md ring-1 ring-cyan-500/20"
                    : "border-slate-200 dark:border-slate-800/90 bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-850 hover:border-slate-300 dark:hover:border-slate-700"
                )}
              >
                <div className="space-y-1 pr-2">
                  ...
                  <p className={cn("text-sm font-bold leading-snug", isSelected ? "text-slate-950 dark:text-white" : "text-slate-800 dark:text-slate-200")}>
                    {role.title}
                  </p>
        ...
        <SpotlightCard className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 dark:border-slate-800/80 pb-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="rounded-full border border-cyan-500/30 bg-cyan-50 dark:bg-cyan-950/60 px-3 py-0.5 text-xs font-bold text-cyan-800 dark:text-cyan-300">
                  {activeRole.company}
                </span>
              </div>
            ...
            <span className="rounded-full border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3.5 py-1 text-xs font-mono font-bold text-slate-800 dark:text-slate-200 shadow-xs">
              {activeRole.period}
            </span>
          </div>
          ...
          {activeRole.metrics && activeRole.metrics.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {activeRole.metrics.map((metric) => (
                <div
                  key={metric}
                  className="flex items-center gap-1.5 rounded-xl border border-emerald-500/40 bg-emerald-50 dark:bg-emerald-950/60 px-3 py-1.5 text-xs font-bold text-emerald-800 dark:text-emerald-300 font-mono shadow-xs"
                >
